...
class RTDETRDetectionModel(DetectionModel):
    ...
    Attributes:
        cfg (str): The configuration file path or preset string. Default is 'rtdetr-r18-l.yaml'.
    ...
    def __init__(self, cfg='rtdetr-r18-l.yaml', ch=3, nc=None, verbose=True):
        """
    ...
    def predict(self, x, profile=False, visualize=False, batch=None, augment=False):
        ...
        for m in self.model[:-1]:  # except the head part
            ...
            if visualize:
                feature_visualization(x, m.type, m.i, save_dir=visualize)
        head = self.model[-1]
        f = head.f if isinstance(head.f, list) else [head.f]
        feats = [y[j] for j in f]
        if len(feats) == 1 and isinstance(feats[0], (list, tuple)):
            feats = feats[0]  # 解包脱掉最外层的括号，防止套娃
        x = head(feats, batch)
        return x
...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in (Classify, Conv, ConvTranspose, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, DSConv, Focus,
                 ...
                 C2f_LWGA, C2f_CSSC, C2f_CNCM, C2f_HFRB, C2f_EVA, C2f_RMBC, C2f_RMBC_LA, C2f_SFMB, HFREB, GDIModule, CASABlock,DSFPNModule,ABFModule,HGRABlock ,MSLCABlock,C2fCIB,CIB,PSA,SCDown,FasterBlock,PConv,
                 DCNv4_Block,HGBlock,HG_Stem,LightConv,ConvBNAct,AFPN_Neck,ASFF,GDNeck,IFM,FAM):
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
                            # ----------------- 替换开始 -----------------
            c2 = args[0]
            if isinstance(f, list):  # 如果 f 是列表（比如 AFPN 的 [5,6,7]）
                c1 = [ch[x] for x in f]
            else:  # 如果 f 是数字（普通模块）
                c1 = ch[f]
                # ----------------- 替换结束 -----------------
